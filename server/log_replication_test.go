...
func up() error {
	...
	wait.ForHTTP(fmt.Sprintf("http://localhost:%d/", consts.KimPort))
	wait.ForHTTP(fmt.Sprintf("http://localhost:%d/", consts.LaszloPort))
	wait.ForHTTP(fmt.Sprintf("http://localhost:%d/", consts.RickyPort))
...
func useTestDbForAll() {
	if err := helpers.Copy("./test data/leader.db", "../db/ricky/log.db"); err != nil {
		log.Print("Couldn't copy", err)
	}
	helpers.Copy("./test data/leader.db", "../db/kim/log.db")
	helpers.Copy("./test data/leader.db", "../db/laszlo/log.db")
	restartAllContainers()
